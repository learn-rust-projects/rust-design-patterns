...
当所有可能的策略在编译期已知时，结合 [enum_dispatch](https://crates.io/crates/enum_dispatch) crate 使用 `enum` 是一种比 `Box<dyn Trait>` 更高性能的替代方案。

- **性能**：避免了动态分发开销（vtable 查找），并允许编译器进行内联优化。
- **内存**：相比堆分配的 trait object，具有更好的缓存局部性。
- **易用性**：`#[enum_dispatch]` 宏会自动为 enum 实现 trait，将调用转发给相应的变体。
