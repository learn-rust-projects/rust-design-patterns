...
static STRATEGIES: LazyLock<Mutex<HashMap<&'static str, StrategyConstructor>>> = LazyLock::new(|| {
...
macro_rules! define_strategy {
    ($struct_name:ident, $name:expr, $pay_fn:expr) => {
        ...
        impl crate::PaymentStrategy for $struct_name {
            fn name(&self) -> &'static str { $name }
            fn pay(&self, amount: f64) -> String { $pay_fn(self, amount) }
        }
        paste! {
        // Auto-register using ctor
        #[ctor::ctor]
        fn [<register_strategy_$struct_name:lower>]() {
            let mut map = crate::STRATEGIES.lock().unwrap();
...
define_strategy!(
    ApplePay,
    "apple_pay",
    |_, amount| format!("Apple Pay: {} yuan", amount)
);

define_strategy!(
    WechatPay,
    "wechat_pay",
    |_, amount| format!("WeChat Pay: {} yuan", amount)
);
